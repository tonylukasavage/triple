...
function runner(command, opts, args, callback) {
	...
	var args = getArgsForCommand(command, opts, args),
		showOutput = false,
		out = '', err = '';
	...
	ti.stdout.on('data', function(data) {
		out += data.toString();
		showOutput && process.stdout.write(data.toString());
		if (/Application started/i.test(data.toString())) {
			showOutput = true;
			return callback();
		}
	});
	ti.stderr.on('data', function(data) {
		err += data.toString();
		showOutput && process.stderr.write(data.toString());
	});
...
function maybeCallback(o) {
	return o && _.isFunction(o) ? o : function(err) { if (err) { throw err; } };
}
