				...
				// process code
					...
					try {
						...
						if ((match = line.match(requireRE)) && !_.contains(opts.module, match[2])) {
							...
							// renaming of the module to deal with commonJS caching
							var cacheBuster = Date.parse(fs.statSync(addPath).mtime);
							var destPath = addPath.replace('.js', '_' + cacheBuster + '.js');
							modulePath = modulePath + '_' + cacheBuster;							
							util.addToApp(addPath, cacheBuster);
...
function load(tokens, rl) {
	var location = tokens[0], delay, watchForChanges; 

	watchForChanges = tokens.indexOf("watch") > 0 ? true : null;

	delay = watchForChanges ? tokens[2] || 0 : tokens[1] || 0;
	...
	// probably a file
		...
		if (fs.existsSync(location)) {
			...
			if (watchForChanges) {
				...
				logger.log('Watching ' + location + ' for updates');							
				fs.watchFile(location, {interval: 250}, function (curr, prev) {
					logger.log(location + ' has changed, restarting app');			  
					return sendLoad();
