...
module.exports = function(opts, callback) {
	...
	async.series([
		...
		function(cb) {
			...
			// set dynamic genymotion host ip
			var interfaces = os.networkInterfaces();
			if (interfaces.vboxnet0) {
				replace({
					regex: '{vboxnet0}',
					replacement: interfaces.vboxnet0[0].address,
					paths: [path.join(resources, 'constants.js')],
					silent: true
				});
			}
			...
			function processQueue() {
				...
				// process code
					...
					try {
						...
						// update paths to file names
						if (opts.platform !== 'ios' && util.fileRx) {
							line = line.replace(util.fileRx, 'Ti.Filesystem.' + constants.DIR_PROP + '+$1');
						}

						// if it contains a require(), but isn't a native require
						if (opts.platform === 'ios' && (match = line.match(requireRE))
							&& !_.contains(opts.module, match[2]))
						{
						...
						// did the line change?
						if (origLine !== line) {
							logger.debug(constants.PROMPT + line);
						}
