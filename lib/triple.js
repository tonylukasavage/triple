		...
		function(cb) {
			if (_.has(opts, 'android')) {
				...
				var buildOpts = _.defaults(opts.build || {}, {
					...
					platform: 'android',
					deviceId: 'Google Nexus S - 4.1.1 - API 16 - 480x800',
			...
			rl.on('line', function(line) {
				...
				// handle triple commands
					...
					switch(match[1].substring(1)) {
						case 'break':
							...
							break;
						case 'clear':
							_.has(opts, 'android') && server.android.emit('message', constants.CLEAR_MESSAGE);
							_.has(opts, 'ios') && server.ios.emit('message', constants.CLEAR_MESSAGE);
							break;
						case 'exit':
							return cb();
							break;
				...
				// process code
					...
					try {
						...
						// send code to server
						_.has(opts, 'android') && server.android.emit('message', code);
						_.has(opts, 'ios') && server.ios.emit('message', code);
